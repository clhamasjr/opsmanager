...
export default function App(){
  ...
  async function handleImport(batch){
    const{error}=await supabase.from('digitacoes').upsert(batch.map(toDb),{onConflict:'proposta,banco',ignoreDuplicates:false})
    if(error){console.error('Upsert err:',error);await supabase.from('digitacoes').insert(batch.map(toDb)).then(({error:e2})=>{if(e2)console.error('Insert err:',e2)})}
  }
