...
/*
 ╔═══════════════════════════════════════════════════════════════════╗
 ║  OpsManager — Sistema de Gestão de Digitações                   ║
 ║  Storage keys: om-ops, om-cad, om-session (PERMANENT)           ║
 ╚═══════════════════════════════════════════════════════════════════╝
*/

// ─── STORAGE ENGINE ──────────────────────────────────────────────
// Keys are PERMANENT. Never rename these.
const STORE = { ops: "om-ops", cad: "om-cad", session: "om-session" };
...
async function dbSet(key, value) {
  try {
    ...
    return true;
  } catch (e) { console.error("[OpsManager] Save failed:", key, e); return false; }
}

// Migration: scan every key we ever used and recover data
async function migrateAll() {
  const opsKeys = ["om-ops","opsmanager-ops","ops-v4","ops-v3","ops-v2","ops-data-v2","ops-data"];
  const cadKeys = ["om-cad","opsmanager-cad","part-v4","part-v3","partners-data-v2","partners-data"];
  let ops = null, cad = null;
  for (const k of opsKeys) {
    if (ops && ops.length) break;
    try { const r = await window.storage.get(k); if (r) { const d = JSON.parse(r.value); if (Array.isArray(d) && d.length) ops = d; } } catch {}
  }
  for (const k of cadKeys) {
    if (cad && cad.length) break;
    try { const r = await window.storage.get(k); if (r) { const d = JSON.parse(r.value); if (Array.isArray(d) && d.length) cad = d; } } catch {}
  }
  // Save to permanent keys
  if (ops && ops.length) await dbSet(STORE.ops, ops);
  if (cad && cad.length) await dbSet(STORE.cad, cad);
  return { ops: ops || [], cad: cad || [] };
}
...
const C = {
  ...
  abg:"#3B82F622",a2bg:"#10B98122",wbg:"#F59E0B18",dbg:"#EF444418",
};
...
// ─── SHARED UI ───────────────────────────────────────────────────
function Btn({children,variant="primary",style,disabled,...p}){const b={border:"none",borderRadius:8,fontFamily:"Outfit",fontWeight:600,fontSize:12,cursor:disabled?"not-allowed":"pointer",padding:"8px 16px",transition:"all .15s",opacity:disabled?.4:1};const v={primary:{background:C.accent,color:"#fff"},success:{background:C.accent2,color:"#fff"},ghost:{background:C.surface,color:C.text,border:"1px solid "+C.border},danger:{background:C.dbg,color:C.danger}};return<button style={{...b,...v[variant],...style}} disabled={disabled} {...p}>{children}</button>;}
...
function usePeriod(){const[per,setPer]=useState("mes");const[df,setDf]=useState("");const[dt,setDt]=useState("");
  const pr=useMemo(()=>{const y=NOW.getFullYear(),m=NOW.getMonth(),fmt=d=>d.toISOString().split("T")[0],f=(a,b)=>fmt(new Date(a,b,1)),l=(a,b)=>fmt(new Date(a,b+1,0));return{mes:{f:f(y,m),t:l(y,m),n:"Mês Atual"},ant:{f:f(y,m-1),t:l(y,m-1),n:"Mês Anterior"},tri:{f:f(y,m-2),t:l(y,m),n:"Trimestre"},sem:{f:f(y,m-5),t:l(y,m),n:"Semestre"},ano:{f:y+"-01-01",t:y+"-12-31",n:String(y)},tudo:{f:"2000-01-01",t:"2099-12-31",n:"Tudo"}};},[]);
  useEffect(()=>{if(per!=="custom"){const p=pr[per];if(p){setDf(p.f);setDt(p.t);}}},[per,pr]);
  return{per,setPer,df,setDf,dt,setDt,pr,filter:ops=>ops.filter(o=>o.data&&o.data>=df&&o.data<=dt),label:per==="custom"?fmtDate(df)+" a "+fmtDate(dt):pr[per]?.n||""};}

function PeriodBar({p}){return(<div style={{display:"flex",flexDirection:"column",gap:6}}>
  <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{Object.entries(p.pr).map(([k,v])=>(<button key={k} onClick={()=>p.setPer(k)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(p.per===k?C.accent:C.border),background:p.per===k?C.abg:"transparent",color:p.per===k?C.accent:C.muted,fontSize:10,fontWeight:p.per===k?600:400,cursor:"pointer",fontFamily:"Outfit"}}>{v.n}</button>))}<button onClick={()=>p.setPer("custom")} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(p.per==="custom"?C.accent:C.border),background:p.per==="custom"?C.abg:"transparent",color:p.per==="custom"?C.accent:C.muted,fontSize:10,cursor:"pointer",fontFamily:"Outfit"}}>Personalizado</button></div>
...
// ─── LOGIN (persisted in storage) ────────────────────────────────
function Login({onLogin}){const[u,setU]=useState("");const[p,setP]=useState("");const[err,setErr]=useState("");
  const go=()=>{if(u.trim().length>=2&&p.trim().length>=2){const sess={name:u.trim(),role:"Gestor",at:Date.now()};dbSet(STORE.session,sess);onLogin(sess);}else setErr("Mín. 2 caracteres");};
  return(<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"linear-gradient(135deg,"+C.bg+",#0D1322)",fontFamily:"Outfit"}}><div style={{background:C.card,border:"1px solid "+C.border,borderRadius:20,padding:"44px 38px",width:370,boxShadow:"0 20px 60px #00000066"}}><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}><div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,"+C.accent+","+C.accent2+")",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:800,color:"#fff"}}>O</div><h1 style={{fontSize:22,fontWeight:800}}>OpsManager</h1></div><p style={{color:C.muted,fontSize:12,marginBottom:28}}>Gestão de Digitações</p>{err&&<div style={{background:C.dbg,color:C.danger,padding:"8px 12px",borderRadius:8,fontSize:12,marginBottom:12}}>{err}</div>}<Field label="Usuário" value={u} onChange={v=>{setU(v);setErr("");}} placeholder="Seu nome"/><div style={{height:8}}/><Field label="Senha" value={p} onChange={v=>{setP(v);setErr("");}} type="password" placeholder="Sua senha"/><div style={{height:16}}/><Btn onClick={go} style={{width:"100%",padding:"11px 0",fontSize:13,borderRadius:10}}>Entrar</Btn></div></div>);}
...
function ImportModal({open,onClose,onImport}){const xl=useSheetJS();const fr=useRef(null);const[step,setStep]=useState(1);const[raw,setRaw]=useState([]);const[hd,setHd]=useState([]);const[mp,setMp]=useState({});const[pv,setPv]=useState([]);const[er,setEr]=useState([]);const[dr,setDr]=useState(false);const[fn,setFn]=useState("");
  const reset=()=>{setStep(1);setRaw([]);setHd([]);setMp({});setPv([]);setEr([]);setFn("");};useEffect(()=>{if(!open)reset();},[open]);
  ...
  return(<div style={{position:"fixed",inset:0,background:"#000000CC",zIndex:300,display:"flex",alignItems:"center",justifyContent:"center",padding:16}} onClick={onClose}><div onClick={e=>e.stopPropagation()} style={{background:C.card,border:"1px solid "+C.border,borderRadius:18,width:760,maxWidth:"97vw",maxHeight:"92vh",overflowY:"auto"}}><div style={{padding:"16px 22px",borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><h3 style={{fontFamily:"Outfit",fontWeight:700,fontSize:15,margin:0}}>Importar Digitações</h3><div style={{display:"flex",gap:10,marginTop:6}}>{["Upload","Mapear","Revisar"].map((s,i)=>(<div key={s} style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:18,height:18,borderRadius:"50%",fontSize:9,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",background:step>i+1?C.accent2:step===i+1?C.accent:C.border,color:step>=i+1?"#fff":C.muted}}>{step>i+1?"✓":i+1}</div><span style={{fontSize:10,color:step===i+1?C.text:C.muted}}>{s}</span></div>))}</div></div><button onClick={onClose} style={{background:"none",border:"none",color:C.muted,fontSize:22,cursor:"pointer"}}>×</button></div>
    <div style={{padding:"16px 22px"}}>
      {step===1&&<div style={{display:"flex",flexDirection:"column",gap:12}}><div onDragOver={e=>{e.preventDefault();setDr(true);}} onDragLeave={()=>setDr(false)} onDrop={e=>{e.preventDefault();setDr(false);const f=e.dataTransfer.files?.[0];if(f)parse(f);}} onClick={()=>fr.current?.click()} style={{border:"2px dashed "+(dr?C.accent:C.border),borderRadius:14,padding:"36px 20px",textAlign:"center",background:dr?C.abg:C.surface,cursor:"pointer"}}><div style={{fontSize:32,marginBottom:6}}>📂</div><div style={{fontSize:13,fontWeight:600}}>{xl?"Arraste ou clique":"Carregando..."}</div><div style={{fontSize:11,color:C.muted}}>Suporta .xlsx, .xls, .csv</div><input ref={fr} type="file" accept=".xlsx,.xls,.csv" onChange={e=>{const f=e.target.files?.[0];if(f)parse(f);}} style={{display:"none"}}/></div>{er.map((e,i)=><div key={i} style={{background:C.dbg,borderRadius:8,padding:"8px 12px",fontSize:12,color:C.danger}}>{e}</div>)}<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:C.surface,borderRadius:10,padding:"10px 14px",border:"1px solid "+C.border}}><span style={{fontSize:12,fontWeight:600}}>📥 Baixar modelo</span><Btn variant="ghost" onClick={dl} style={{fontSize:11}}>Baixar</Btn></div></div>}
...
function Dashboard({ops}){const p=usePeriod();const f=p.filter(ops);const tR=f.reduce((s,o)=>s+(o.vrRepasse||0),0);const fin=f.filter(isFinal);const fR=fin.reduce((s,o)=>s+(o.vrRepasse||0),0);const ags=[...new Set(f.map(o=>o.agente).filter(Boolean))];const tB=f.reduce((s,o)=>s+(o.vrBruto||0),0);
  ...
  const byOp=useMemo(()=>{const m={};f.forEach(o=>{const op=o.operacao||"?";if(!m[op])m[op]={r:0,c:0};m[op].r+=(o.vrRepasse||0);m[op].c++;});return Object.entries(m).sort((a,b)=>b[1].r-a[1].r);},[f]);
...
// ─── KANBAN ──────────────────────────────────────────────────────
function Kanban({ops,setOps}){const per=usePeriod();const[fa,sfa]=useState("");const[fo,sfo]=useState("");const[fb,sfb]=useState("");const[dt,sdt]=useState(null);const base=per.filter(ops);const fd=base.filter(o=>(!fa||o.agente===fa)&&(!fo||o.operacao===fo)&&(!fb||o.banco===fb));
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:12}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Kanban</h2><PeriodBar p={per}/>
    ...
    {sits.length===0?<div style={{background:C.card,borderRadius:14,padding:28,textAlign:"center",color:C.muted}}>Importe digitações</div>:<div style={{display:"flex",gap:7,overflowX:"auto",paddingBottom:6}}>{sits.map(sit=>{const it=fd.filter(o=>o.situacao===sit);const sc=sitColor(sit);const r=it.reduce((s,o)=>s+(o.vrRepasse||0),0);return(<div key={sit} style={{minWidth:190,flex:1,background:C.surface,borderRadius:10,border:"1px solid "+C.border}}><div style={{padding:"8px 10px",borderBottom:"1px solid "+C.border,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><span style={{fontSize:11,fontWeight:700,color:sc}}>{sit}</span><div style={{fontSize:8,color:C.muted}}>{fmtCur(r)}</div></div><span style={{fontSize:9,background:sc+"22",color:sc,padding:"2px 6px",borderRadius:6,fontWeight:700}}>{it.length}</span></div><div style={{padding:4,display:"flex",flexDirection:"column",gap:3,maxHeight:360,overflowY:"auto"}}>{it.slice(0,25).map(o=>(<div key={o.id} onClick={()=>sdt(o)} style={{background:C.card,border:"1px solid "+C.border,borderRadius:7,padding:"6px 8px",cursor:"pointer"}}><div style={{fontSize:10,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{o.cliente||o.proposta}</div><div style={{fontSize:8,color:C.muted}}>{o.banco}·{o.operacao}·{fmtCur(o.vrRepasse)}</div><div style={{fontSize:7,color:C.muted}}>{o.agente}</div><div style={{display:"flex",gap:2,marginTop:3,flexWrap:"wrap"}}>{sits.filter(s=>s!==sit).slice(0,3).map(s=>(<button key={s} onClick={e=>{e.stopPropagation();setOps(pv=>pv.map(x=>x.id===o.id?{...x,situacao:s}:x));}} style={{fontSize:7,padding:"1px 3px",borderRadius:3,border:"1px solid "+C.border,background:C.surface,color:sitColor(s),cursor:"pointer",fontWeight:600}}>→{s}</button>))}</div></div>))}{it.length===0&&<div style={{padding:10,textAlign:"center",color:C.muted,fontSize:9}}>—</div>}</div></div>);})}</div>}
    <Modal open={!!dt} onClose={()=>sdt(null)} title="Detalhes" width={460}>{dt&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,fontSize:12}}>{[["Cliente",dt.cliente],["Banco",dt.banco],["Op.",dt.operacao],["Situação",dt.situacao],["Agente",dt.agente],["Repasse",fmtCur(dt.vrRepasse)],["Data",fmtDate(dt.data)]].map(([l,v])=>(<div key={l}><div style={{fontSize:8,color:C.muted,fontWeight:600,textTransform:"uppercase"}}>{l}</div><div style={{fontWeight:500}}>{v||"—"}</div></div>))}</div>}</Modal>
  </div>);}

// ─── CADASTROS ───────────────────────────────────────────────────
function Cadastros({ops,cad,setCad}){const per=usePeriod();const f=per.filter(ops);const xl=useSheetJS();const fr=useRef(null);const[se,sSe]=useState("");const[fSt,sFSt]=useState("");
  const opsAg=[...new Set(ops.map(o=>o.agente).filter(Boolean))];const miss=opsAg.filter(a=>!cad.some(c=>c.name.toLowerCase()===a.toLowerCase()));
  const autoImp=()=>{const nw=miss.map(a=>({id:uid(),name:a,contato:"",obs:"",status:"Ativo"}));if(nw.length)setCad(p=>[...p,...nw]);};
  const impFile=file=>{if(!xl||!window.XLSX)return;const rd=new FileReader();rd.onload=e=>{try{const wb=window.XLSX.read(new Uint8Array(e.target.result),{type:"array"});const rows=window.XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]],{defval:""});const cols=Object.keys(rows[0]||{});const nc=cols.find(c=>/nome|parceiro|agente|name/i.test(c));const cc=cols.find(c=>/contato|telefone|email|fone/i.test(c));const oc=cols.find(c=>/obs|nota/i.test(c));const sc=cols.find(c=>/status|situação|ativo/i.test(c));const ex=new Set(cad.map(c=>c.name.toLowerCase()));const nw=[];rows.forEach(r=>{const n=nc?String(r[nc]||"").trim():"";if(n&&!ex.has(n.toLowerCase())){ex.add(n.toLowerCase());nw.push({id:uid(),name:n,contato:cc?String(r[cc]||"").trim():"",obs:oc?String(r[oc]||"").trim():"",status:sc?String(r[sc]||"Ativo").trim():"Ativo"});}});if(nw.length)setCad(p=>[...p,...nw]);}catch(e){console.error(e);}};rd.readAsArrayBuffer(file);};
  const stats=useMemo(()=>cad.map(c=>{const al=f.filter(o=>o.agente?.toLowerCase()===c.name.toLowerCase());const aT=ops.filter(o=>o.agente?.toLowerCase()===c.name.toLowerCase());const r=al.reduce((s,o)=>s+(o.vrRepasse||0),0);const fn=al.filter(isFinal);const lo=[...aT].sort((a,b)=>(b.data||"").localeCompare(a.data||""))[0];const ds=lo?Math.floor((NOW-new Date(lo.data))/86400000):999;let cp="Ativo";if(al.length===0&&ds>60)cp="Improdutivo";else if(al.length===0&&ds>30)cp="Inativo";else if(al.length===0)cp="Sem produção";return{...c,count:al.length,r,fC:fn.length,cv:al.length?(fn.length/al.length*100):0,ld:lo?.data,ds,cp,tA:aT.length};}).sort((a,b)=>b.r-a.r),[cad,f,ops]);
  const bySt=useMemo(()=>{const m={};stats.forEach(s=>{m[s.cp]=(m[s.cp]||0)+1;});return Object.entries(m).filter(([,v])=>v>0);},[stats]);
  const fd=stats.filter(s=>(!se||s.name.toLowerCase().includes(se.toLowerCase()))&&(!fSt||s.cp===fSt));
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Cadastros</h2><div style={{display:"flex",gap:6}}>{miss.length>0&&<Btn onClick={autoImp} style={{background:C.wbg,color:C.warn,border:"1px solid "+C.warn+"44",fontSize:11}}>⚡ {miss.length} das digitações</Btn>}<Btn variant="ghost" onClick={()=>fr.current?.click()} style={{fontSize:11}}>📥 Importar planilha</Btn><input ref={fr} type="file" accept=".xlsx,.xls,.csv" onChange={e=>{const f=e.target.files?.[0];if(f)impFile(f);}} style={{display:"none"}}/></div></div>
    <PeriodBar p={per}/>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Stat label="Cadastrados" value={cad.length}/>{bySt.map(([s,c])=><Stat key={s} label={s} value={c} color={s==="Ativo"?C.accent2:s==="Improdutivo"?C.danger:s==="Inativo"?C.warn:C.muted}/>)}<Stat label="Com Produção" value={stats.filter(s=>s.count>0).length} color={C.accent}/></div>
    {cad.length>0&&<div style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:14}}><div style={{display:"flex",height:22,borderRadius:6,overflow:"hidden"}}>{bySt.map(([s,c])=>{const col=s==="Ativo"?C.accent2:s==="Improdutivo"?C.danger:s==="Inativo"?C.warn:C.muted;const pct=cad.length?(c/cad.length*100):0;return pct>0?<div key={s} style={{width:pct+"%",background:col,display:"flex",alignItems:"center",justifyContent:"center",fontSize:8,color:"#fff",fontWeight:600}} title={s}>{pct>10?s+" ("+c+")":c}</div>:null;})}</div></div>}
    <div style={{display:"flex",gap:6}}><input value={se} onChange={e=>sSe(e.target.value)} placeholder="Buscar..." style={{background:C.surface,border:"1px solid "+C.border,borderRadius:7,color:C.text,padding:"7px 12px",fontSize:12,outline:"none",flex:1,minWidth:160,fontFamily:"Outfit"}}/><Field value={fSt} onChange={sFSt} options={bySt.map(([s])=>s)} style={{minWidth:110}}/></div>
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+C.border}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:C.surface}}>{["Parceiro","Status","Dig.","Repasse","Pagas","Conv.","Última","Dias","Total"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:C.muted,fontSize:8,textTransform:"uppercase"}}>{h}</th>)}</tr></thead><tbody>{fd.map(s=>{const col=s.cp==="Ativo"?C.accent2:s.cp==="Improdutivo"?C.danger:s.cp==="Inativo"?C.warn:C.muted;return(<tr key={s.id} style={{borderBottom:"1px solid "+C.border}}><td style={{padding:"8px 10px",fontWeight:600}}>{s.name}</td><td style={{padding:"8px 10px"}}><Badge text={s.cp} color={col}/></td><td style={{padding:"8px 10px"}}>{s.count}</td><td style={{padding:"8px 10px",fontWeight:600,color:C.accent}}>{fmtCur(s.r)}</td><td style={{padding:"8px 10px",color:C.accent2}}>{s.fC}</td><td style={{padding:"8px 10px"}}><span style={{color:s.cv>=50?C.accent2:s.cv>=30?C.warn:C.danger,fontWeight:600}}>{s.cv.toFixed(0)}%</span></td><td style={{padding:"8px 10px"}}>{s.ld?fmtDate(s.ld):"Nunca"}</td><td style={{padding:"8px 10px",color:s.ds>60?C.danger:s.ds>30?C.warn:C.text}}>{s.ds<999?s.ds+"d":"—"}</td><td style={{padding:"8px 10px"}}>{s.tA}</td></tr>);})}</tbody></table>{fd.length===0&&<div style={{padding:24,textAlign:"center",color:C.muted}}>Importe parceiros.</div>}</div>
  </div>);}

// ─── ESTRATÉGICO ─────────────────────────────────────────────────
function Estrategico({ops,cad}){const per=usePeriod();const f=per.filter(ops);const[sel,sSel]=useState(null);
  const ags=[...new Set(ops.map(o=>o.agente).filter(Boolean))];
  const list=useMemo(()=>ags.map(a=>{const al=f.filter(o=>o.agente===a),aT=ops.filter(o=>o.agente===a),r=al.reduce((s,o)=>s+(o.vrRepasse||0),0),fn=al.filter(isFinal),fR=fn.reduce((s,o)=>s+(o.vrRepasse||0),0),cv=al.length?(fn.length/al.length*100):0;
    const bO={};al.forEach(o=>{const k=o.operacao||"?";if(!bO[k])bO[k]={c:0,r:0,fc:0};bO[k].c++;bO[k].r+=(o.vrRepasse||0);if(isFinal(o))bO[k].fc++;});
    const bB={};al.forEach(o=>{const k=o.banco||"?";if(!bB[k])bB[k]={c:0,r:0,fc:0};bB[k].c++;bB[k].r+=(o.vrRepasse||0);if(isFinal(o))bB[k].fc++;});
    ...
    return{name:a,c:al.length,r,fR,fC:fn.length,cv,bO:Object.entries(bO).sort((x,y)=>y[1].r-x[1].r),bB:Object.entries(bB).sort((x,y)=>y[1].r-x[1].r),bS:Object.entries(bS).sort((x,y)=>y[1].c-x[1].c),mo:Object.entries(mo).sort((x,y)=>x[0].localeCompare(y[0])).slice(-6),ld:lo?.data,tA:aT.length,ct:cad.find(c=>c.name.toLowerCase()===a.toLowerCase())?.contato||""};
  }).sort((a,b)=>b.r-a.r),[ags,f,ops,cad]);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Visão Estratégica</h2><PeriodBar p={per}/>
    {!s?<div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+C.border}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:C.surface}}>{["Parceiro","Dig.","Repasse","Pagas","Conv.","Última",""].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:C.muted,fontSize:8,textTransform:"uppercase"}}>{h}</th>)}</tr></thead><tbody>{list.map(p=>(<tr key={p.name} style={{borderBottom:"1px solid "+C.border,cursor:"pointer"}} onClick={()=>sSel(p.name)}><td style={{padding:"8px 10px",fontWeight:600}}>{p.name}</td><td style={{padding:"8px 10px"}}>{p.c}</td><td style={{padding:"8px 10px",fontWeight:600,color:C.accent}}>{fmtCur(p.r)}</td><td style={{padding:"8px 10px",color:C.accent2}}>{p.fC}</td><td style={{padding:"8px 10px"}}><span style={{color:p.cv>=50?C.accent2:p.cv>=30?C.warn:C.danger,fontWeight:600}}>{p.cv.toFixed(0)}%</span></td><td style={{padding:"8px 10px"}}>{p.ld?fmtDate(p.ld):"—"}</td><td style={{padding:"8px 10px",color:C.accent}}>→</td></tr>))}</tbody></table>{list.length===0&&<div style={{padding:24,textAlign:"center",color:C.muted}}>Importe digitações</div>}</div>
    :<div style={{display:"flex",flexDirection:"column",gap:12}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}><button onClick={()=>sSel(null)} style={{background:C.surface,border:"1px solid "+C.border,borderRadius:8,color:C.accent,padding:"5px 10px",cursor:"pointer",fontSize:11,fontFamily:"Outfit"}}>← Voltar</button><h3 style={{fontFamily:"Outfit",fontWeight:700,fontSize:17}}>{s.name}</h3>{s.ct&&<span style={{fontSize:10,color:C.muted}}>{s.ct}</span>}</div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Stat label="Digitações" value={s.c}/><Stat label="Repasse" value={fmtCur(s.r)} color={C.accent}/><Stat label="Pagas" value={s.fC} sub={fmtCur(s.fR)} color={C.accent2}/><Stat label="Conv." value={s.cv.toFixed(1)+"%"} color={s.cv>=50?C.accent2:s.cv>=30?C.warn:C.danger}/><Stat label="Total" value={s.tA}/></div>
      ...
      <div style={{background:C.card,border:"1px solid "+C.accent+"33",borderRadius:14,padding:16}}><div style={{fontSize:12,fontWeight:700,color:C.accent,marginBottom:8}}>📌 Pontos para Conversa</div><div style={{display:"flex",flexDirection:"column",gap:4,fontSize:11}}>
        ...
        {s.cv<30&&s.c>0&&<div style={{color:C.danger}}>⚠ Conversão baixa ({s.cv.toFixed(0)}%) — revisar estornos</div>}
        {s.cv>=30&&s.cv<50&&s.c>0&&<div style={{color:C.warn}}>⚡ Conversão razoável — alinhar bancos</div>}
        {s.cv>=50&&s.c>0&&<div style={{color:C.accent2}}>✓ Boa conversão — aumentar volume</div>}
        {s.bO.length>0&&<div>📊 Principal: <strong>{s.bO[0][0]}</strong> ({s.bO[0][1].c} dig, {fmtCur(s.bO[0][1].r)})</div>}
        {s.bB.length>0&&<div>🏦 Banco: <strong>{s.bB[0][0]}</strong></div>}
        {s.bS.find(([x])=>x==="ESTORNADO")&&<div style={{color:C.danger}}>⚠ {s.bS.find(([x])=>x==="ESTORNADO")[1].c} estornos</div>}
        {s.mo.length>=2&&(()=>{const l=s.mo[s.mo.length-1][1].r,p=s.mo[s.mo.length-2][1].r,v=p?((l-p)/p*100):0;return v<-20?<div style={{color:C.danger}}>📉 Queda de {Math.abs(v).toFixed(0)}%</div>:v>20?<div style={{color:C.accent2}}>📈 +{v.toFixed(0)}%</div>:null;})()}
      </div></div>
    </div>}
  </div>);}

// ─── PRODUÇÃO (Banco / Convênio / Operação) ─────────────────────
function Producao({ops}){const per=usePeriod();const f=per.filter(ops);const[tab,setTab]=useState("banco");const tR=f.reduce((s,o)=>s+(o.vrRepasse||0),0);const tB=f.reduce((s,o)=>s+(o.vrBruto||0),0);const tFin=f.filter(isFinal);const tFR=tFin.reduce((s,o)=>s+(o.vrRepasse||0),0);
  const data=useMemo(()=>{const keyFn=tab==="banco"?o=>o.banco:tab==="convenio"?o=>o.convenio:o=>o.operacao;const subFn=tab==="operacao"?o=>o.banco:o=>o.operacao;
    const m={};f.forEach(o=>{const k=keyFn(o)||"SEM INFO";if(!m[k])m[k]={c:0,r:0,br:0,fc:0,fr:0,subs:{},prc:{}};m[k].c++;m[k].r+=(o.vrRepasse||0);m[k].br+=(o.vrBruto||0);if(isFinal(o)){m[k].fc++;m[k].fr+=(o.vrRepasse||0);}const sub=subFn(o)||"?";if(!m[k].subs[sub])m[k].subs[sub]={c:0,r:0,fc:0};m[k].subs[sub].c++;m[k].subs[sub].r+=(o.vrRepasse||0);if(isFinal(o))m[k].subs[sub].fc++;const ag=o.agente||"?";if(!m[k].prc[ag])m[k].prc[ag]={c:0,r:0,fc:0};m[k].prc[ag].c++;m[k].prc[ag].r+=(o.vrRepasse||0);if(isFinal(o))m[k].prc[ag].fc++;});
    return Object.entries(m).sort((a,b)=>b[1].r-a[1].r);},[f,tab]);
  const subLbl=tab==="operacao"?"Bancos":"Operações";const tabs=[{id:"banco",n:"Por Banco",ic:"🏦"},{id:"convenio",n:"Por Convênio",ic:"📑"},{id:"operacao",n:"Por Operação",ic:"⚡"}];
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Produção</h2><PeriodBar p={per}/>
    <div style={{display:"flex",gap:4}}>{tabs.map(t=>(<button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"6px 14px",borderRadius:8,border:"1px solid "+(tab===t.id?C.accent:C.border),background:tab===t.id?C.abg:"transparent",color:tab===t.id?C.accent:C.muted,fontSize:11,fontWeight:tab===t.id?600:400,cursor:"pointer",fontFamily:"Outfit"}}>{t.ic} {t.n}</button>))}</div>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Stat label="Repasse" value={fmtCur(tR)} color={C.accent}/><Stat label="Pago" value={fmtCur(tFR)} color={C.accent2} sub={tFin.length+" ops"}/><Stat label="Bruto" value={fmtCur(tB)} color={C.info}/><Stat label="Digitações" value={f.length}/><Stat label={tab==="banco"?"Bancos":tab==="convenio"?"Convênios":"Operações"} value={data.length}/></div>
    {/* Ranking */}
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+C.border}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:C.surface}}>{[tab==="banco"?"Banco":tab==="convenio"?"Convênio":"Operação","Dig.","Repasse","%","Pago","Conv.","Bruto","Top Parceiro"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:C.muted,fontSize:8,textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{data.map(([n,d])=>{const pct=tR?(d.r/tR*100):0;const cv=d.c?(d.fc/d.c*100):0;const tp=Object.entries(d.prc).sort((a,b)=>b[1].r-a[1].r)[0];return(<tr key={n} style={{borderBottom:"1px solid "+C.border}}><td style={{padding:"8px 10px",fontWeight:700}}>{n}</td><td style={{padding:"8px 10px"}}>{d.c}</td><td style={{padding:"8px 10px",fontWeight:600,color:C.accent}}>{fmtCur(d.r)}</td><td style={{padding:"8px 10px"}}><div style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:40,height:5,background:C.surface,borderRadius:2}}><div style={{height:"100%",background:C.accent,borderRadius:2,width:pct+"%"}}/></div><span style={{fontSize:9,color:C.muted}}>{pct.toFixed(1)}%</span></div></td><td style={{padding:"8px 10px",color:C.accent2,fontWeight:600}}>{fmtCur(d.fr)}</td><td style={{padding:"8px 10px"}}><span style={{fontWeight:600,color:cv>=50?C.accent2:cv>=30?C.warn:C.danger}}>{cv.toFixed(0)}%</span></td><td style={{padding:"8px 10px"}}>{fmtCur(d.br)}</td><td style={{padding:"8px 10px",fontSize:10}}>{tp?tp[0]:"—"}</td></tr>);})}</tbody></table>{data.length===0&&<div style={{padding:24,textAlign:"center",color:C.muted}}>Sem dados</div>}</div>
    {/* Cards */}
    {data.length>0&&<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(290px,1fr))",gap:10}}>{data.slice(0,12).map(([n,d])=>{const cv=d.c?(d.fc/d.c*100):0;const subs=Object.entries(d.subs).sort((a,b)=>b[1].r-a[1].r);const tops=Object.entries(d.prc).sort((a,b)=>b[1].r-a[1].r).slice(0,5);return(<div key={n} style={{background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:14}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:13,fontWeight:700}}>{n}</span><span style={{fontSize:10,color:C.muted}}>{d.c} dig</span></div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10}}><div><div style={{fontSize:8,color:C.muted,fontWeight:600}}>REPASSE</div><div style={{fontSize:12,fontWeight:700,color:C.accent}}>{fmtCur(d.r)}</div></div><div><div style={{fontSize:8,color:C.muted,fontWeight:600}}>PAGO</div><div style={{fontSize:12,fontWeight:700,color:C.accent2}}>{fmtCur(d.fr)}</div></div><div><div style={{fontSize:8,color:C.muted,fontWeight:600}}>CONV.</div><div style={{fontSize:12,fontWeight:700,color:cv>=50?C.accent2:cv>=30?C.warn:C.danger}}>{cv.toFixed(0)}%</div></div></div>
      <div style={{fontSize:10,fontWeight:600,color:C.muted,marginBottom:3}}>{subLbl}</div>
      {subs.slice(0,5).map(([sb,sd])=>(<div key={sb} style={{display:"flex",alignItems:"center",gap:5,padding:"2px 0"}}><span style={{fontSize:10,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{sb}</span><span style={{fontSize:8,color:C.muted}}>{sd.c}</span><span style={{fontSize:9,fontWeight:600,minWidth:60,textAlign:"right"}}>{fmtCur(sd.r)}</span></div>))}
      {tops.length>0&&<><div style={{fontSize:10,fontWeight:600,color:C.muted,marginTop:6,marginBottom:3}}>Top Parceiros</div>{tops.map(([ag,ad],i)=>(<div key={ag} style={{display:"flex",alignItems:"center",gap:4,padding:"1px 0"}}><span style={{fontSize:9,color:i<3?C.accent:C.muted,fontWeight:700,width:10}}>{i+1}</span><span style={{fontSize:10,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{ag}</span><span style={{fontSize:9,fontWeight:600}}>{fmtCur(ad.r)}</span></div>))}</>}
    </div>);})}</div>}
  </div>);}
...
function Portabilidade({ops}){const per=usePeriod();const f=per.filter(ops);const port=useMemo(()=>f.filter(o=>(o.operacao||"").toUpperCase()==="PORTABILIDADE"),[f]);
  ...
  const TB=({d,nl})=>(<div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{background:C.surface}}>{[nl,"Dig.","Pago","Conv.","Rep. Dig.","Rep. Pago"].map(h=><th key={h} style={{padding:"7px 9px",textAlign:"left",fontWeight:600,color:C.muted,fontSize:8,textTransform:"uppercase"}}>{h}</th>)}</tr></thead><tbody>{d.map(([n,x])=>{const r=x.d?(x.p/x.d*100):0;return(<tr key={n} style={{borderBottom:"1px solid "+C.border}}><td style={{padding:"7px 9px",fontWeight:600,maxWidth:180,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{n}</td><td style={{padding:"7px 9px"}}>{x.d}</td><td style={{padding:"7px 9px",color:C.accent2,fontWeight:600}}>{x.p}</td><td style={{padding:"7px 9px"}}><RB r={r}/></td><td style={{padding:"7px 9px"}}>{fmtCur(x.rd)}</td><td style={{padding:"7px 9px",fontWeight:600,color:C.accent2}}>{fmtCur(x.rp)}</td></tr>);})}</tbody></table></div>);
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Portabilidade</h2><PeriodBar p={per}/>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Stat label="Digitado" value={tD} sub={fmtCur(tRD)}/><Stat label="Pago" value={tP} sub={fmtCur(tRP)} color={C.accent2}/><Stat label="Conv." value={cv.toFixed(1)+"%"} color={cv>=50?C.accent2:cv>=30?C.warn:C.danger}/></div>
...
// ─── ANÁLISE ─────────────────────────────────────────────────────
function Analise({ops}){const per=usePeriod();const f=per.filter(ops);const ags=[...new Set(ops.map(o=>o.agente).filter(Boolean))];const cM=NOW.toISOString().slice(0,7);const pM=new Date(NOW.getFullYear(),NOW.getMonth()-1,1).toISOString().slice(0,7);
  const st=ags.map(a=>{const al=f.filter(o=>o.agente===a);const cu=ops.filter(o=>o.agente===a&&o.data?.startsWith(cM));const pv=ops.filter(o=>o.agente===a&&o.data?.startsWith(pM));const cR=cu.reduce((s,o)=>s+(o.vrRepasse||0),0),pR=pv.reduce((s,o)=>s+(o.vrRepasse||0),0);const vr=pR?((cR-pR)/pR*100):(cR>0?100:0);const lo=[...ops.filter(o=>o.agente===a)].sort((a,b)=>(b.data||"").localeCompare(a.data||""))[0];const ds=lo?Math.floor((NOW-new Date(lo.data))/86400000):999;const fn=al.filter(isFinal);let a2="ok";if(cu.length===0&&pv.length>0)a2="inactive";else if(vr<=-30)a2="drop";else if(vr<0)a2="soft";return{nm:a,cc:cu.length,pc:pv.length,cR,pR,vr,ds,ld:lo?.data,a2,fc:al.length,fr:al.reduce((s,o)=>s+(o.vrRepasse||0),0),cv:al.length?(fn.length/al.length*100):0};}).sort((a,b)=>{const o={inactive:0,drop:1,soft:2,ok:3};return(o[a.a2]??4)-(o[b.a2]??4);});
  const ac=st.filter(s=>["inactive","drop"].includes(s.a2)).length;
  const days=[];for(let i=29;i>=0;i--){const d=new Date(NOW);d.setDate(d.getDate()-i);const ds=d.toISOString().split("T")[0];const dOps=ops.filter(o=>o.data===ds);days.push({d:ds,c:dOps.length});}const mxC=Math.max(...days.map(d=>d.c),1);
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}><h2 style={{fontFamily:"Outfit",fontWeight:800,fontSize:20}}>Análise</h2><PeriodBar p={per}/>
    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}><Stat label="Mês Atual" value={ops.filter(o=>o.data?.startsWith(cM)).length} sub={ops.filter(o=>o.data?.startsWith(pM)).length+" ant."}/><Stat label="Em Queda" value={st.filter(s=>["drop","soft"].includes(s.a2)).length} color={C.warn}/><Stat label="Sem Digitar" value={st.filter(s=>s.a2==="inactive").length} color={C.danger}/><Stat label="Alertas" value={ac} color={ac?C.danger:C.accent2}/></div>
    {ac>0&&<div style={{background:C.dbg,border:"1px solid "+C.danger+"33",borderRadius:12,padding:12}}><div style={{fontSize:11,fontWeight:700,color:C.danger,marginBottom:4}}>⚠ Atenção</div>{st.filter(s=>["inactive","drop"].includes(s.a2)).map(s=>(<div key={s.nm} style={{fontSize:10,padding:"2px 0"}}><span style={{color:s.a2==="inactive"?C.danger:C.warn}}>{s.a2==="inactive"?"🔴":"🟡"}</span> <strong>{s.nm}</strong> <span style={{color:C.muted}}>— {s.a2==="inactive"?"Parado (última: "+(s.ld?fmtDate(s.ld):"nunca")+")":"Queda "+Math.abs(s.vr).toFixed(0)+"%"}</span></div>))}</div>}
    ...
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid "+C.border}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:10}}><thead><tr style={{background:C.surface}}>{["","Parceiro","Período","Repasse","Mês","Ant.","Var.","Conv.","Última","Dias"].map(h=><th key={h} style={{padding:"7px 8px",textAlign:"left",fontWeight:600,color:C.muted,fontSize:8,textTransform:"uppercase"}}>{h}</th>)}</tr></thead><tbody>{st.map(s=>{const ic=s.a2==="inactive"?"🔴":s.a2==="drop"?"🔴":s.a2==="soft"?"🟡":"↗";const vc=s.vr>0?C.accent2:s.vr<-30?C.danger:s.vr<0?C.warn:C.text;return(<tr key={s.nm} style={{borderBottom:"1px solid "+C.border}}><td style={{padding:"7px 8px",fontSize:12}}>{ic}</td><td style={{padding:"7px 8px",fontWeight:600,maxWidth:160,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.nm}</td><td style={{padding:"7px 8px"}}>{s.fc}</td><td style={{padding:"7px 8px",fontWeight:600,color:C.accent}}>{fmtCur(s.fr)}</td><td style={{padding:"7px 8px"}}>{s.cc}</td><td style={{padding:"7px 8px"}}>{s.pc}</td><td style={{padding:"7px 8px",fontWeight:600,color:vc}}>{s.vr>0?"+":""}{s.vr.toFixed(0)}%</td><td style={{padding:"7px 8px"}}><span style={{color:s.cv>=50?C.accent2:s.cv>=30?C.warn:C.danger,fontWeight:600}}>{s.cv.toFixed(0)}%</span></td><td style={{padding:"7px 8px"}}>{s.ld?fmtDate(s.ld):"—"}</td><td style={{padding:"7px 8px",color:s.ds>30?C.danger:s.ds>14?C.warn:C.text}}>{s.ds<999?s.ds+"d":"—"}</td></tr>);})}</tbody></table></div>
  </div>);}
...
const NAV=[
  ...
  {id:"kanban",l:"Kanban",i:"📋"},
  {id:"cadastros",l:"Cadastros",i:"📁"},
  {id:"estrategico",l:"Estratégico",i:"🤝"},
  {id:"producao",l:"Produção",i:"🏦"},
  {id:"portabilidade",l:"Portabilidade",i:"🔄"},
  {id:"analise",l:"Análise",i:"📈"},
];

export default function App() {
  const [user, setUser] = useState(undefined); // undefined=loading, null=logged out, obj=logged in
  const [view, setView] = useState("dashboard");
  const [ops, setOpsR] = useState([]);
  const [cad, setCadR] = useState([]);
  const [loaded, setLoaded] = useState(false);
  const [saving, setSaving] = useState(false);

  // Persist ops with feedback
  const setOps = useCallback(fn => {
    setOpsR(prev => {
      const next = typeof fn === "function" ? fn(prev) : fn;
      setSaving(true);
      dbSet(STORE.ops, next).then(() => setTimeout(() => setSaving(false), 500));
      return next;
    });
  }, []);

  // Persist cad with feedback
  const setCad = useCallback(fn => {
    setCadR(prev => {
      const next = typeof fn === "function" ? fn(prev) : fn;
      dbSet(STORE.cad, next);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    (async () => {
      // 1. Restore session
      const sess = await dbGet(STORE.session);
      if (sess && sess.name) {
        setUser(sess);
      } else {
        setUser(null);
      ...
      // 2. Load data (try primary keys first, then migrate)
      let o = await dbGet(STORE.ops);
      let c = await dbGet(STORE.cad);

      if ((!o || !o.length) || (!c || !c.length)) {
        const migrated = await migrateAll();
        if (!o || !o.length) o = migrated.ops;
        if (!c || !c.length) c = migrated.cad;
      }

      setOpsR(Array.isArray(o) ? o : []);
      setCadR(Array.isArray(c) ? c : []);
      setLoaded(true);
    })();
  }, []);
  ...
  if (user === undefined || !loaded) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: C.bg, color: C.text, fontFamily: "Outfit" }}>
        <div style={{ textAlign: "center" }}>
          <div style={{ width: 40, height: 40, borderRadius: 10, background: "linear-gradient(135deg," + C.accent + "," + C.accent2 + ")", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, fontWeight: 800, color: "#fff", margin: "0 auto 12px" }}>O</div>
          <div style={{ fontSize: 14, fontWeight: 600 }}>Carregando...</div>
          <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>Recuperando seus dados</div>
        </div>
      </div>
    );
  }
  ...
  const inact = ags.filter(a => !ops.some(o => o.agente === a && o.data?.startsWith(cM))).length;
    ...
    <>
      ...
      <div style={{ display: "flex", minHeight: "100vh" }}>
        ...
        <div style={{ width: 200, background: C.card, borderRight: "1px solid " + C.border, display: "flex", flexDirection: "column", flexShrink: 0 }}>
          <div style={{ padding: "20px 16px 12px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <div style={{ width: 28, height: 28, borderRadius: 7, background: "linear-gradient(135deg," + C.accent + "," + C.accent2 + ")", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#fff" }}>O</div>
              <h1 style={{ fontSize: 15, fontWeight: 800 }}>OpsManager</h1>
            </div>
            <div style={{ fontSize: 9, color: C.muted, marginTop: 2, marginLeft: 36 }}>{ops.length} dig · {cad.length} cad</div>
          </div>
          <nav style={{ flex: 1, padding: "2px 8px" }}>
            {NAV.map(n => {
              const a = view === n.id;
              const badge = n.id === "analise" && inact > 0;
              return (
                <button key={n.id} onClick={() => setView(n.id)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 10px", marginBottom: 1, borderRadius: 8, border: "none", background: a ? C.abg : "transparent", color: a ? C.accent : C.muted, fontFamily: "Outfit", fontSize: 11, fontWeight: a ? 600 : 400, cursor: "pointer", textAlign: "left" }}>
                  <span style={{ fontSize: 14 }}>{n.i}</span>{n.l}
                  {badge && <span style={{ marginLeft: "auto", background: C.danger, color: "#fff", fontSize: 8, fontWeight: 700, padding: "1px 5px", borderRadius: 6 }}>{inact}</span>}
                </button>
              );
            })}
          </nav>
          <div style={{ padding: "12px 16px", borderTop: "1px solid " + C.border }}>
            ...
            <div style={{ fontSize: 9, color: C.muted, marginBottom: 5 }}>{user.role}</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={logout} style={{ fontSize: 9, color: C.danger, background: "none", border: "none", cursor: "pointer", padding: 0 }}>Sair →</button>
              {saving && <span style={{ fontSize: 8, color: C.accent2 }}>💾 Salvando...</span>}
            </div>
          </div>
        </div>
        {/* Content */}
        <div style={{ flex: 1, padding: "22px 26px", overflowY: "auto", maxWidth: "calc(100vw - 200px)" }}>
          ...
          {view === "estrategico" && <Estrategico ops={ops} cad={cad} />}
