...
function isRecebido(o) { return o.crcCliente && o.dataNossoCredito }
...
function filterByPeriod(ops, per) {
  const r = PERIODS[per] || PERIODS.tudo
  return ops.filter(o => o.data && o.data >= r.f && o.data <= r.t)
}

function PeriodBar({ per, setPer }) {
  return (
    <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginBottom: 6 }}>
      {Object.entries(PERIODS).map(([k, v]) => (
        <button key={k} onClick={() => setPer(k)} style={{
          ...
          fontWeight: per === k ? 700 : 400, cursor: 'pointer'
        }}>{v.n}</button>
      ))}
    </div>
  )
...
/* ═══ LOAD ALL RECORDS (Supabase default is 1000 — we paginate) ═══ */
async function loadAllDigitacoes() {
  const PAGE = 5000
  let all = []
  let from = 0
  let keepGoing = true
...
function Dashboard({ ops }) {
  ...
  const f = filterByPeriod(ops, per)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <PeriodBar per={per} setPer={setPer} />
      <div style={{ fontSize: 10, color: C.muted }}>Período: <strong style={{ color: C.text }}>{PERIODS[per]?.n}</strong> — {f.length} digitações filtradas de {ops.length}</div>
      ...
      <div style={{ background: C.card, border: '1px solid ' + C.accent + '44', borderRadius: 14, padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 10, color: C.muted }}>{proj.duP}/{proj.duT} DU ({pctDU.toFixed(0)}%) · Restam <strong style={{ color: C.accent }}>{proj.duR}</strong></div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 10 }}>
          ...
          <div><div style={{ fontSize: 8, color: C.muted, fontWeight: 600 }}>PAGO MÊS</div><div style={{ fontSize: 15, fontWeight: 700, color: C.accent2 }}>{fmtCur(proj.fR)}</div><div style={{ fontSize: 9, color: C.muted }}>{proj.fC} finalizadas</div></div>
        </div>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Situação</div>
          {sitArr.slice(0, 8).map(([s, c]) => <div key={s} style={{ marginBottom: 4 }}><div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10 }}><span style={{ color: sitCol(s), fontWeight: 600 }}>{s}</span><span style={{ color: C.muted }}>{c}</span></div><div style={{ height: 4, background: C.surface, borderRadius: 2 }}><div style={{ height: '100%', background: sitCol(s), borderRadius: 2, width: (c / (f.length || 1) * 100) + '%' }} /></div></div>)}
        </div>
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Top Parceiros</div>
          {topP.map(([ag, d], i) => <div key={ag} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '3px 0', borderBottom: '1px solid ' + C.border }}><span style={{ fontSize: 10, fontWeight: 700, color: i < 3 ? C.accent : C.muted, width: 16 }}>{i + 1}</span><div style={{ flex: 1, fontSize: 10, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ag}</div><span style={{ fontSize: 10, fontWeight: 700, color: C.accent2 }}>{fmtCur(d.r)}</span></div>)}
        </div>
      </div>
    </div>
  )
...
function Producao({ ops }) {
  ...
  const f = filterByPeriod(ops, per)
  const tR = f.reduce((s, o) => s + (o.vrRepasse || 0), 0)
  const data = (() => { const kFn = tab === 'banco' ? o => o.banco : tab === 'convenio' ? o => o.convenio : o => o.operacao; const m = {}; f.forEach(o => { const k = kFn(o) || '?'; if (!m[k]) m[k] = { c: 0, r: 0, fc: 0, fr: 0 }; m[k].c++; m[k].r += (o.vrRepasse || 0); if (isFin(o)) { m[k].fc++; m[k].fr += (o.vrRepasse || 0) } }); return Object.entries(m).sort((a, b) => b[1].r - a[1].r) })()
  const proj = getProjection(ops)
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ background: C.card, border: '1px solid ' + C.accent + '44', borderRadius: 14, padding: 16 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}><span style={{ fontSize: 12, fontWeight: 700, color: C.accent }}>📊 Posição + Projeção</span><span style={{ fontSize: 10, color: C.muted }}>{proj.duP}/{proj.duT} DU · Restam {proj.duR}</span></div>
      ...
      <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid ' + C.border }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr style={{ background: C.surface }}>{[tab === 'banco' ? 'Banco' : tab === 'convenio' ? 'Convênio' : 'Operação', 'Dig.', 'Repasse', 'Pago', 'Conv.'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: C.muted, fontSize: 8, textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{data.map(([n, d]) => { const cv = d.c ? (d.fc / d.c * 100) : 0; return <tr key={n} style={{ borderBottom: '1px solid ' + C.border }}><td style={{ padding: '8px 10px', fontWeight: 700 }}>{n}</td><td style={{ padding: '8px 10px' }}>{d.c}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: C.accent }}>{fmtCur(d.r)}</td><td style={{ padding: '8px 10px', color: C.accent2, fontWeight: 600 }}>{fmtCur(d.fr)}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: cv >= 50 ? C.accent2 : cv >= 30 ? C.warn : C.danger }}>{cv.toFixed(0)}%</td></tr> })}</tbody></table></div>
    </div>
  )
}
...
function Estrategico({ ops }) {
  ...
  const f = filterByPeriod(ops, per)
  const list = (() => { const ags = [...new Set(f.map(o => o.agente).filter(Boolean))]; return ags.map(a => { const al = f.filter(o => o.agente === a); const fn = al.filter(isFin); const est = al.filter(isEst); const r = al.reduce((s, o) => s + (o.vrRepasse || 0), 0); const cv = al.length ? (fn.length / al.length * 100) : 0; const er = al.length ? (est.length / al.length * 100) : 0; const bO = {}; al.forEach(o => { const k = o.operacao || '?'; if (!bO[k]) bO[k] = { c: 0, r: 0 }; bO[k].c++; bO[k].r += (o.vrRepasse || 0) }); const bB = {}; al.forEach(o => { const k = o.banco || '?'; if (!bB[k]) bB[k] = { c: 0, r: 0 }; bB[k].c++; bB[k].r += (o.vrRepasse || 0) }); const lo = [...ops.filter(o => o.agente === a)].sort((a, b) => (b.data || '').localeCompare(a.data || ''))[0]; return { name: a, c: al.length, r, fC: fn.length, cv, estC: est.length, er, bO: Object.entries(bO).sort((a, b) => b[1].r - a[1].r), bB: Object.entries(bB).sort((a, b) => b[1].r - a[1].r), ld: lo?.data } }).sort((a, b) => b.r - a.r) })()
  const s = sel ? list.find(l => l.name === sel) : null
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <h2 style={{ fontWeight: 800, fontSize: 20 }}>Estratégico</h2>
      <PeriodBar per={per} setPer={setPer} />
      {!s ? <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid ' + C.border }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr style={{ background: C.surface }}>{['Parceiro', 'Dig.', 'Repasse', 'Conv.', 'Estornos', 'Última', ''].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: C.muted, fontSize: 8, textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{list.map(p => <tr key={p.name} style={{ borderBottom: '1px solid ' + C.border, cursor: 'pointer' }} onClick={() => sSel(p.name)}><td style={{ padding: '8px 10px', fontWeight: 600 }}>{p.name}</td><td style={{ padding: '8px 10px' }}>{p.c}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: C.accent }}>{fmtCur(p.r)}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: p.cv >= 50 ? C.accent2 : p.cv >= 30 ? C.warn : C.danger }}>{p.cv.toFixed(0)}%</td><td style={{ padding: '8px 10px', color: p.estC ? C.danger : C.muted }}>{p.estC} ({p.er.toFixed(0)}%)</td><td style={{ padding: '8px 10px' }}>{p.ld ? fmtDate(p.ld) : '—'}</td><td style={{ color: C.accent }}>→</td></tr>)}</tbody></table></div>
      : <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Stat label="Digitações" value={s.c} /><Stat label="Repasse" value={fmtCur(s.r)} color={C.accent} /><Stat label="Conv." value={s.cv.toFixed(0) + '%'} color={s.cv >= 50 ? C.accent2 : s.cv >= 30 ? C.warn : C.danger} /><Stat label="Estornos" value={s.estC} sub={s.er.toFixed(0) + '%'} color={s.estC ? C.danger : C.muted} /></div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}><div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Operações</div>{s.bO.map(([op, d]) => <div key={op} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, padding: '3px 0' }}><span style={{ fontWeight: 600 }}>{op} ({d.c})</span><span style={{ color: C.accent }}>{fmtCur(d.r)}</span></div>)}</div>
          <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}><div style={{ fontSize: 12, fontWeight: 600, marginBottom: 8 }}>Bancos</div>{s.bB.map(([b, d]) => <div key={b} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, padding: '3px 0' }}><span style={{ fontWeight: 600 }}>{b} ({d.c})</span><span style={{ color: C.accent }}>{fmtCur(d.r)}</span></div>)}</div>
        </div>
        <div style={{ background: C.card, border: '1px solid ' + C.accent + '33', borderRadius: 14, padding: 16 }}><div style={{ fontSize: 12, fontWeight: 700, color: C.accent, marginBottom: 8 }}>📌 Pontos para Conversa</div><div style={{ fontSize: 11, display: 'flex', flexDirection: 'column', gap: 4 }}>{s.cv < 30 && s.c > 0 && <div style={{ color: C.danger }}>⚠ Conversão baixa ({s.cv.toFixed(0)}%)</div>}{s.cv >= 50 && s.c > 0 && <div style={{ color: C.accent2 }}>✓ Boa conversão</div>}{s.er > 20 && <div style={{ color: C.danger }}>⚠ {s.estC} estornos ({s.er.toFixed(0)}%) — URGENTE</div>}{s.bO[0] && <div>Principal op: <strong>{s.bO[0][0]}</strong></div>}{s.bB[0] && <div>Principal banco: <strong>{s.bB[0][0]}</strong></div>}</div></div>
      </div>}
    </div>
  )
}
...
function Ranking({ ops }) {
  const [per, setPer] = useState('tudo')
  const f = filterByPeriod(ops, per)
  const data = (() => { const ags = [...new Set(f.map(o => o.agente).filter(Boolean))]; return ags.map(a => { const al = f.filter(o => o.agente === a); const fn = al.filter(isFin); const est = al.filter(isEst); const r = al.reduce((s, o) => s + (o.vrRepasse || 0), 0); const cv = al.length ? (fn.length / al.length * 100) : 0; const cu = ops.filter(o => o.agente === a && o.data?.startsWith(CUR_M)); const pv = ops.filter(o => o.agente === a && o.data?.startsWith(PREV_M)); const cuR = cu.reduce((s, o) => s + (o.vrRepasse || 0), 0); const pvR = pv.reduce((s, o) => s + (o.vrRepasse || 0), 0); const trend = pvR ? ((cuR - pvR) / pvR * 100) : (cuR > 0 ? 100 : 0); return { name: a, c: al.length, r, cv, estC: est.length, cuC: cu.length, cuR, trend } }).sort((a, b) => b.r - a.r) })()
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}><h2 style={{ fontWeight: 800, fontSize: 20 }}>Ranking</h2><PeriodBar per={per} setPer={setPer} /><div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid ' + C.border }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr style={{ background: C.surface }}>{['#', 'Parceiro', 'Dig.', 'Repasse', 'Conv.', 'Estornos', 'Mês', 'Tendência'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: C.muted, fontSize: 8, textTransform: 'uppercase' }}>{h}</th>)}</tr></thead><tbody>{data.map((d, i) => <tr key={d.name} style={{ borderBottom: '1px solid ' + C.border }}><td style={{ padding: '8px 10px' }}><span style={{ display: 'inline-flex', width: 22, height: 22, borderRadius: 6, background: i < 3 ? C.accent : C.surface, alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: i < 3 ? '#fff' : C.muted }}>{i + 1}</span></td><td style={{ padding: '8px 10px', fontWeight: 600 }}>{d.name}</td><td style={{ padding: '8px 10px' }}>{d.c}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: C.accent }}>{fmtCur(d.r)}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: d.cv >= 50 ? C.accent2 : d.cv >= 30 ? C.warn : C.danger }}>{d.cv.toFixed(0)}%</td><td style={{ padding: '8px 10px', color: d.estC ? C.danger : C.muted }}>{d.estC || '0'}</td><td style={{ padding: '8px 10px' }}>{d.cuC} · {fmtCur(d.cuR)}</td><td style={{ padding: '8px 10px', fontWeight: 600, color: d.trend > 0 ? C.accent2 : d.trend < -20 ? C.danger : C.warn }}>{d.trend > 0 ? '+' : ''}{d.trend.toFixed(0)}%</td></tr>)}</tbody></table></div></div>
  )
}
...
function Recebimentos({ ops }) {
  ...
  const f = filterByPeriod(ops, per)
  const pend = f.filter(isPendReceb), rec = f.filter(isRecebido)
  ...
  const byAg = (() => { const m = {}; pend.forEach(o => { const a = o.agente || '?'; if (!m[a]) m[a] = { c: 0, r: 0 }; m[a].c++; m[a].r += (o.vrRepasse || 0) }); return Object.entries(m).sort((a, b) => b[1].r - a[1].r) })()
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <PeriodBar per={per} setPer={setPer} />
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}><Stat label="Pendentes" value={pend.length} sub={fmtCur(pR)} color={C.danger} /><Stat label="Recebidas" value={rec.length} sub={fmtCur(rec.reduce((s, o) => s + (o.vrRepasse || 0), 0))} color={C.accent2} /><Stat label="% Pendente" value={f.filter(o => o.crcCliente).length ? (pend.length / f.filter(o => o.crcCliente).length * 100).toFixed(0) + '%' : '—'} color={C.warn} /></div>
      {pend.length > 0 && <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}><div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10 }}>Por Operação</div><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr style={{ background: C.surface }}>{['Operação', 'Qtd', 'Pendente'].map(h => <th key={h} style={{ padding: '7px 9px', textAlign: 'left', color: C.muted, fontSize: 8 }}>{h}</th>)}</tr></thead><tbody>{byOp.map(([op, d]) => <tr key={op} style={{ borderBottom: '1px solid ' + C.border }}><td style={{ padding: '7px 9px', fontWeight: 600 }}>{op}</td><td style={{ padding: '7px 9px' }}>{d.c}</td><td style={{ padding: '7px 9px', fontWeight: 600, color: C.danger }}>{fmtCur(d.r)}</td></tr>)}</tbody></table></div>
        </div>
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}><div style={{ fontSize: 12, fontWeight: 700, marginBottom: 10 }}>Por Parceiro</div><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}><thead><tr style={{ background: C.surface }}>{['Parceiro', 'Qtd', 'Pendente'].map(h => <th key={h} style={{ padding: '7px 9px', textAlign: 'left', color: C.muted, fontSize: 8 }}>{h}</th>)}</tr></thead><tbody>{byAg.map(([a, d]) => <tr key={a} style={{ borderBottom: '1px solid ' + C.border }}><td style={{ padding: '7px 9px', fontWeight: 600 }}>{a}</td><td style={{ padding: '7px 9px' }}>{d.c}</td><td style={{ padding: '7px 9px', fontWeight: 600, color: C.danger }}>{fmtCur(d.r)}</td></tr>)}</tbody></table></div>
        <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16 }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}><span style={{ fontSize: 12, fontWeight: 700 }}>Analítico</span><select value={fB} onChange={e => sFB(e.target.value)} style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 6, color: C.text, padding: '4px 8px', fontSize: 10 }}><option value="">— Banco —</option>{[...new Set(pend.map(o => o.banco).filter(Boolean))].sort().map(b => <option key={b} value={b}>{b}</option>)}</select></div><div style={{ fontSize: 10, color: C.muted, marginBottom: 6 }}>{filt.length} pendências — {fmtCur(filt.reduce((s, o) => s + (o.vrRepasse || 0), 0))}</div><div style={{ overflowX: 'auto', maxHeight: 350, borderRadius: 8, border: '1px solid ' + C.border }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 10 }}><thead><tr style={{ background: C.surface, position: 'sticky', top: 0 }}>{['Cliente', 'Banco', 'Op.', 'Agente', 'Repasse', 'CRC', 'Dias'].map(h => <th key={h} style={{ padding: '6px 8px', textAlign: 'left', color: C.muted, fontSize: 8 }}>{h}</th>)}</tr></thead><tbody>{filt.slice(0, 300).map(o => { const d = o.crcCliente ? Math.floor((NOW - new Date(o.crcCliente)) / 86400000) : 0; return <tr key={o.id} style={{ borderBottom: '1px solid ' + C.border }}><td style={{ padding: '5px 8px' }}>{o.cliente}</td><td style={{ padding: '5px 8px' }}>{o.banco}</td><td style={{ padding: '5px 8px' }}>{o.operacao}</td><td style={{ padding: '5px 8px' }}>{o.agente}</td><td style={{ padding: '5px 8px', fontWeight: 600, color: C.danger }}>{fmtCur(o.vrRepasse)}</td><td style={{ padding: '5px 8px' }}>{fmtDate(o.crcCliente)}</td><td style={{ padding: '5px 8px', fontWeight: 600, color: d > 90 ? C.danger : d > 30 ? C.warn : C.text }}>{d}d</td></tr> })}</tbody></table></div></div>
      </>}
    </div>
  )
}
...
function Alertas({ ops }) {
  ...
  const st = ags.map(a => { const cu = ops.filter(o => o.agente === a && o.data?.startsWith(CUR_M)); const pv = ops.filter(o => o.agente === a && o.data?.startsWith(PREV_M)); const cR = cu.reduce((s, o) => s + (o.vrRepasse || 0), 0); const pR = pv.reduce((s, o) => s + (o.vrRepasse || 0), 0); const vr = pR ? ((cR - pR) / pR * 100) : (cR > 0 ? 100 : 0); const lo = [...ops.filter(o => o.agente === a)].sort((a, b) => (b.data || '').localeCompare(a.data || ''))[0]; const ds = lo ? Math.floor((NOW - new Date(lo.data)) / 86400000) : 999; let flag = 'ok'; if (cu.length === 0 && pv.length > 0) flag = 'parado'; else if (vr <= -30) flag = 'queda'; return { nm: a, cc: cu.length, pc: pv.length, cR, vr, ds, ld: lo?.data, flag } }).sort((a, b) => { const o = { parado: 0, queda: 1, ok: 2 }; return (o[a.flag] ?? 3) - (o[b.flag] ?? 3) })
  return (
...
function Usuarios({ user }) {
  ...
  async function criarUsuario(e) {
    e.preventDefault()
    if (!nome || !email || !senha) { setMsg('Preencha todos os campos'); return }
    const { error } = await supabase.from('usuarios').insert({ nome, email, senha, perfil })
    if (error) { setMsg('Erro: ' + error.message); return }
    setMsg('Usuário criado!')
    setNome(''); setEmail(''); setSenha(''); setShowNew(false)
  ...
  async function alterarPerfil(u, novoPerfil) {
    await supabase.from('usuarios').update({ perfil: novoPerfil }).eq('id', u.id)
    const { data } = await supabase.from('usuarios').select('*').order('nome')
    setUsers(data || [])
  }

  if (user.perfil !== 'admin') return <div style={{ padding: 28, textAlign: 'center', color: C.muted }}>Acesso restrito a administradores</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2 style={{ fontWeight: 800, fontSize: 20 }}>Usuários</h2>
        <button onClick={() => setShowNew(!showNew)} style={{ background: C.accent, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontWeight: 600, fontSize: 12, cursor: 'pointer' }}>+ Novo Usuário</button>
      </div>
      {msg && <div style={{ background: C.accent + '22', color: C.accent, padding: '8px 12px', borderRadius: 8, fontSize: 12 }}>{msg}</div>}
      {showNew && (
        <form onSubmit={criarUsuario} style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 14, padding: 16, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr auto', gap: 10, alignItems: 'end' }}>
        ...
        <div style={{ overflowX: 'auto', borderRadius: 10, border: '1px solid ' + C.border }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
            <thead><tr style={{ background: C.surface }}>
              {['Nome', 'Email', 'Perfil', 'Status', 'Último Acesso', 'Ações'].map(h => <th key={h} style={{ padding: '8px 10px', textAlign: 'left', color: C.muted, fontSize: 8, textTransform: 'uppercase' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {users.map(u => (
                <tr key={u.id} style={{ borderBottom: '1px solid ' + C.border }}>
                  ...
                  <td style={{ padding: '8px 10px' }}>
                    <select value={u.perfil} onChange={e => alterarPerfil(u, e.target.value)} style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 4, color: C.text, padding: '2px 6px', fontSize: 10 }}>
                      <option value="operador">Operador</option><option value="gestor">Gestor</option><option value="admin">Admin</option>
                    </select>
                  </td>
                  <td style={{ padding: '8px 10px' }}><Badge text={u.ativo ? 'Ativo' : 'Inativo'} color={u.ativo ? C.accent2 : C.danger} /></td>
                  <td style={{ padding: '8px 10px', fontSize: 10, color: C.muted }}>{u.ultimo_acesso ? new Date(u.ultimo_acesso).toLocaleString('pt-BR') : 'Nunca'}</td>
                  <td style={{ padding: '8px 10px' }}><button onClick={() => toggleAtivo(u)} style={{ background: u.ativo ? '#EF444418' : C.accent2 + '22', color: u.ativo ? C.danger : C.accent2, border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 10, fontWeight: 600, cursor: 'pointer' }}>{u.ativo ? 'Desativar' : 'Ativar'}</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function getNav(perfil) {
  const levels = { operador: 1, gestor: 2, admin: 3 }
  const userLevel = levels[perfil] || 1
  return NAV_ALL.filter(n => (levels[n.min] || 1) <= userLevel)
}
...
export default function App() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('om-session')
      if (saved) {
        const u = JSON.parse(saved)
        if (u && u.nome) setUser(u)
      }
    } catch (e) { /* ignore */ }
  }, [])
  ...
  useEffect(() => {
    ...
    setStatus('loading')
    loadAllDigitacoes()
      .then(all => { setOps(all); setStatus('') })
      .catch(() => setStatus('error'))
  }, [user])

  // LOGIN against usuarios table
  ...
  async function handleImport(rows) {
    ...
    try {
      ...
      const all = await loadAllDigitacoes()
      setOps(all)
      setStatus('')
      ...
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.bg, fontFamily: 'Outfit,sans-serif', color: C.text }}>
        <form onSubmit={handleLogin} style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: 20, padding: '40px 36px', width: 380 }}>
          ...
          <div style={{ marginBottom: 8 }}>
            <label style={{ fontSize: 9, color: C.muted, fontWeight: 600, display: 'block', marginBottom: 3 }}>EMAIL</label>
            <input name="email" type="email" required placeholder="seu@email.com" autoComplete="email" style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 7, color: C.text, padding: '10px 12px', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: 'Outfit,sans-serif' }} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={{ fontSize: 9, color: C.muted, fontWeight: 600, display: 'block', marginBottom: 3 }}>SENHA</label>
            <input name="senha" type="password" required placeholder="Sua senha" style={{ background: C.surface, border: '1px solid ' + C.border, borderRadius: 7, color: C.text, padding: '10px 12px', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box', fontFamily: 'Outfit,sans-serif' }} />
          </div>
      ...
      <div style={{ width: 195, background: C.card, borderRight: '1px solid ' + C.border, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
        <div style={{ padding: '20px 14px 10px' }}>
          ...
          <div style={{ fontSize: 9, color: C.muted, marginTop: 2, marginLeft: 33 }}>
            {ops.length} dig {status === 'loading' ? '· ⏳' : status === 'saving' ? '· 💾' : status === 'error' ? '· ⚠' : ''}
          </div>
        ...
        <nav style={{ flex: 1, padding: '2px 7px', overflowY: 'auto' }}>
          {nav.map(n => (
            ...
            }}><span style={{ fontSize: 13 }}>{n.i}</span>{n.l}</button>
          ))}
        </nav>
        <div style={{ padding: '10px 14px', borderTop: '1px solid ' + C.border }}>
          <div style={{ fontSize: 11, fontWeight: 600 }}>{user.nome}</div>
          <div style={{ fontSize: 9, color: C.muted, marginBottom: 4 }}>{user.perfil}</div>
          <button onClick={handleLogout} style={{ fontSize: 9, color: C.danger, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>Sair →</button>
        </div>
      </div>
