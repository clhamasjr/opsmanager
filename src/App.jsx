  ...
  return<div style={{display:'flex',flexDirection:'column',gap:14}}>
    ...
    {R&&<>
      ...
      {fase&&(()=>{const f=NC_FASES.find(x=>x.id===fase);const list=R.byFase[fase].rows
        ...
        <div style={{overflowX:'auto',borderRadius:10,border:'1px solid '+C.border,maxHeight:400,overflowY:'auto'}}><table style={{width:'100%',borderCollapse:'collapse'}}><thead><tr style={{background:C.surface,position:'sticky',top:0}}>{['Proposta','Cliente','Valor','Situação','Detalhe banco','Dias','Digitador','📝 Última observação'].map(h=><th key={h} style={th}>{h}</th>)}</tr></thead><tbody>
          {list.slice(0,300).map((r,i)=><tr key={i}><td style={td}>{r.proposta}</td><td style={td}>{(r.cliente||'—').slice(0,20)}</td><td style={{...td,fontWeight:600}}>{cfMoney(r.vr_bruto)}</td><td style={td}><Badge text={r.situacao||'—'} color={f.color}/></td><td style={{...td,fontSize:10,fontWeight:600,color:sbCol(r.situacao_banco)}}>{r.situacao_banco||'—'}</td><td style={{...td,fontWeight:700,color:agCol(r._dias)}}>{r._dias!=null?r._dias+'d':'—'}</td><td style={{...td,color:C.muted,fontSize:10}}>{(r.usuario||'—').slice(0,16)}</td><td style={{...td,fontSize:10,maxWidth:320}} title={r.obs_texto?(r.obs_autor+' · '+fmtDate(r.obs_data)+': '+r.obs_texto):''}>{r.obs_texto?<span><b style={{color:C.accent}}>{(r.obs_autor||'').split(' ')[0]}:</b> {r.obs_texto.slice(0,90)}{r.obs_texto.length>90?'…':''}</span>:<span style={{color:C.muted}}>—</span>}</td></tr>)}
        </tbody></table></div>
      </div>})()}
